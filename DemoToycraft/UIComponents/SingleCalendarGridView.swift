//
//  SingleCalendarGridView.swift
//  DemoToycraft
//
//  Created by iquest1127 on 2022/05/17.
//

import Foundation
import SwiftUI

struct SingleCalendarGridView<DateView>: View where DateView: View {
                ...
                HStack{
                    Button {
                        guard let prevMonth = calendar.date(byAdding: .month, value: -1, to: currentPage),
                              interval.contains(calendar.startOfDay(for: prevMonth))
                        else { return }
                        withAnimation(.easeOut){
                            scrollViewProxy.scrollTo(DateCellComponent(date: calendar.startOfDay(for: prevMonth), component: .month))
                            currentPage = calendar.startOfDay(for: prevMonth)
                        }
                    ...
                    Button {
                        guard let nextMonth = calendar.date(byAdding: .month, value: 1, to: currentPage),
                              interval.contains(calendar.startOfDay(for: nextMonth))
                        else { return }
                        withAnimation(.easeIn) {
                            scrollViewProxy.scrollTo(DateCellComponent(date: calendar.startOfDay(for: nextMonth), component: .month))
                            currentPage = calendar.startOfDay(for: nextMonth)
                        }
                    ...
                    ScrollView(.horizontal) {
                        LazyHStack{
                            ForEach(monthList, id: \.self) { month in
                                SingleCalendarMonthView(month: month.date, content: content)
                                    .id(month).frame(width: geomtryProxy.size.width, height: geomtryProxy.size.height)
                            }
                        }
                    }
                }
...
struct SingleCalendarMonthView<DateView>: View where DateView: View {
    @Environment(\.calendar) private var calendar
    @Environment(\.timeZone) private var timeZone
    @Environment(\.locale) private var locale
    let month:Date
    ...
    var body: some View {
        LazyVGrid(columns: Array(repeating: .init(), count: calendar.weekdaySymbols.count)) {
            Section {
...
struct SingleCalendarGridView_Previews: PreviewProvider {
